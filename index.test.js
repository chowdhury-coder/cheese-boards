...
describe("Cheese Board Test", () => {
    ...
    test("Test: Eager Loading - A user can be loaded with its boards", async ()=>{
        const user = await User.create({
            name: "Mohammed Chowdhury",
            email: "mohammad.chowdhury@clover.com"
        })
        ...
        expect(userBoards.Boards[0].type).toBe("Glass")
        expect(userBoards.Boards[1].type).toBe("Bronze")
        expect(userBoards.Boards[2].type).toBe("Gold")
        expect(userBoards.Boards.length).toEqual(3)        
